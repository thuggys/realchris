...
export default function Collection() {
  ...
  const initialCollection = searchParams.get('type') || 'seasonal';
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      if (heroRef.current) {
        ...
        // Diagonal reveal
        const diagonal = heroRef.current.querySelector('.diagonal-reveal');
        gsap.fromTo(
          diagonal,
          { scaleX: 0, opacity: 0 },
  ...
  useEffect(() => {
    if (isLoading || typeof window === 'undefined' || !heroRef.current) return;
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const bgImage = heroRef.current?.querySelector('.hero-bg-image');
      const particles = heroRef.current?.querySelector('.particles-container');
  ...
  return (
    <div className="bg-background min-h-screen">
      ...
      <main className="min-h-screen">
        ...
        <section ref={heroRef} className="relative h-[calc(100vh-102px)] mt-[102px] overflow-hidden">
          ...
          <div ref={particlesRef} className="particles-container absolute inset-0 z-10 overflow-hidden">
